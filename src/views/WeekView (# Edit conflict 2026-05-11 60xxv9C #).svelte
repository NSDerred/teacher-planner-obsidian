<script lang="ts">
  ...
  export function prevWeek()      { currentDate = addWeeks(currentDate, -1); }
  export function nextWeek()      { currentDate = addWeeks(currentDate, 1); }
  ...
  $: _slotMap = (() => {
    ...
    for (const day of DAYS) {
      ...
      if (!tmpl) continue;
      const visible = !abEnabled || !abWeekType
        ? tmpl.slots
        : tmpl.slots.filter(s => s.weekType === abWeekType || s.weekType === "both" || s.weekType == null);
  ...
  // ── Context menu ──────────────────────────────────────────────────────────
  let contextMenu: { x: number; y: number; slot: TimetableSlot } | null = null;

  function onChipContextMenu(e: MouseEvent, slot: TimetableSlot) {
    e.preventDefault();
    e.stopPropagation();
    contextMenu = { x: e.clientX, y: e.clientY, slot };
  }

  function closeContextMenu() { contextMenu = null; }

  // ── Action menu (••• button for short rows — uses Obsidian Menu) ─────────
  function showActionMenu(e: MouseEvent, items: { icon: string; label: string; fn: () => void }[]) {
    e.stopPropagation();
    closeContextMenu();
    const menu = new Menu();
    for (const item of items) {
      menu.addItem(i => i.setTitle(item.icon + " " + item.label).onClick(item.fn));
    }
  ...
  function openNotesModal(slot: TimetableSlot) {
    const lbl = getSlotLabel(slot);
    new SlotNotesModal(plugin.app, plugin, slot.id, lbl.notes, lbl.classroom, () => invalidate()).open();
  }

  function onChipClick(slot: TimetableSlot) {
    if (contextMenu) { closeContextMenu(); return; }
    openNotesModal(slot);
  }

  function contextEditNotes() {
  ...
  async function contextRemoveSlot() {
    if (!contextMenu) return;
    const slotId = contextMenu.slot.id;
    closeContextMenu();
  ...
  // ── Navigation ────────────────────────────────────────────────────────────
  function onPrev()  { currentDate = addWeeks(currentDate, -1); }
  function onNext()  { currentDate = addWeeks(currentDate, 1); }
  function onToday() { currentDate = new Date(); }
  ...
  // Mode toggle in header (light ↔ dark only — theme style set in Settings)
  function onModeToggle() {
    plugin.settings.themeMode = plugin.settings.themeMode === "dark" ? "light" : "dark";
    plugin.saveSettings();
    invalidate();
  }
  $: modeLabel = plugin.settings.themeMode === "light" ? "🌙 Dark" : "☀ Light";
  ...
  // ── Week notes ────────────────────────────────────────────────────────────
  const DEFAULT_NOTES_HEIGHT = 120;
  let notesHeight   = plugin.settings.notesHeight ?? DEFAULT_NOTES_HEIGHT;
  let resizerActive = false;
  let resizerStartY = 0;
  let resizerStartH = 0;

  function onResizerMouseDown(e: MouseEvent) {
    resizerActive = true; resizerStartY = e.clientY; resizerStartH = notesHeight;
    e.preventDefault();
  }
  function onWindowMouseMove(e: MouseEvent) {
    if (!resizerActive) return;
    notesHeight = Math.max(60, Math.min(600, resizerStartH + (resizerStartY - e.clientY)));
  }
  async function onWindowMouseUp() {
    if (!resizerActive) return;
    resizerActive = false;
    plugin.settings.notesHeight = notesHeight;
    await plugin.saveSettings();
  }

  function getNoteValue(): string { return (plugin.settings.weekNotes ?? {})[currentWeekKey] ?? ""; }
  async function setNoteValue(v: string) {
  ...
  function onWindowClick(e: MouseEvent) {
    ...
    if (contextMenu && !t.closest(".tp-context-menu")) closeContextMenu();
  }
</script>
...
<div class="tp-week-view" data-tp-theme={plugin.settings.theme ?? "carbon"} data-tp-mode={plugin.settings.themeMode ?? "dark"}>
  ...
  <header class="tp-header">
    ...
    <nav class="tp-nav" aria-label="Week navigation">
      <button class="tp-btn" on:click={onPrev} aria-label="Previous week">← Prev</button>
      <button class="tp-btn tp-btn-accent" on:click={onToday}>This Week</button>
      <button class="tp-btn" on:click={onNext} aria-label="Next week">Next →</button>
    </nav>
    <div class="tp-header-actions">
      <button class="tp-btn" on:click={() => onAddEvent()} aria-label="Add event">＋ Event</button>
      <button class="tp-btn" on:click={onOpenTimetable} aria-label="Edit timetable">🗓 Timetable</button>
      <button class="tp-btn" on:click={onOpenSettings} aria-label="Settings">⚙ Settings</button>
      <button class="tp-btn tp-btn-theme" on:click={onModeToggle} title="Toggle light/dark mode">{modeLabel}</button>
    </div>
  </header>
    ...
    <table class="tp-grid" style="--grid-colour:{plugin.settings.gridLineColour ?? '#555'}; --grid-weight:{plugin.settings.gridLineWeight ?? 1}px; --block-colour:{plugin.settings.blockBorderColour ?? '#444'}; --block-weight:{plugin.settings.blockBorderWeight ?? 1}px;">
      ...
      <tbody>
        ...
        {#each _periods as period (period.id)}
          ...
          {@const isShort = rh < 70}
          <tr class="tp-period-row" style="height:{rh}px; border-top:var(--block-weight) solid var(--block-colour); border-bottom:var(--block-weight) solid var(--block-colour);">
            <td class="tp-td-time">{period.start}</td>
                  ...
                  <div class="tp-event-stack">
                    {#if slot}
                      ...
                      <div
                        ...
                        on:click={() => onChipClick(slot)}
                        on:contextmenu={(e) => onChipContextMenu(e, slot)}
                        on:keydown={(e) => e.key === "Enter" && onChipClick(slot)}
                      ...
                      >
                        ...
                        {/if}
                        {#if lbl.notes && !isShort}
                          <span class="tp-chip-notes">{lbl.notes}</span>
                        {/if}
                        {#if isShort}
                          <button class="tp-chip-dots" title="More actions"
                            on:click|stopPropagation={(e) => showActionMenu(e, [
                              { icon: "📝", label: "Notes",       fn: () => openNotesModal(slot) },
                              { icon: "📓", label: "Lesson note", fn: () => openOrCreateLessonNote(slot, dayDate) },
                              { icon: "+",  label: "Add event",   fn: () => openEventPickerDirect(dayDate, period.id) },
                            ])}>···</button>
                        {:else}
                          <div class="tp-chip-actions">
                            <button class="tp-chip-action-btn" title="Notes"
                              on:click|stopPropagation={() => openNotesModal(slot)}>📝</button>
                            <button class="tp-chip-action-btn" title="Open/create lesson note"
                              on:click|stopPropagation={() => openOrCreateLessonNote(slot, dayDate)}>📓</button>
                            <button class="tp-chip-action-btn" title="Add one-off event"
                              on:click|stopPropagation={(e) => openEventPicker(e, dayDate, period.id)}>+</button>
                          </div>
                        {/if}
                      </div>
                    {/if}
                    {#each devEvents as devEv (devEv.id)}
                      ...
                      <div
                        ...
                        on:dragend={onDragEnd}
                        on:click={() => onEditDateEvent(devEv)}
                        on:keydown={(e) => e.key === "Enter" && onEditDateEvent(devEv)}
                      ...
                      >
                        ...
                        {/if}
                        {#if lbl.notes && !isShort}<span class="tp-chip-notes">{lbl.notes}</span>{/if}
                        {#if isShort}
                          <button class="tp-chip-dots" title="More actions"
                            on:click|stopPropagation={(e) => showActionMenu(e, [
                              { icon: "📝", label: "Edit event",  fn: () => onEditDateEvent(devEv) },
                              { icon: "📓", label: "Lesson note", fn: () => openOrCreateLessonNoteForEvent(devEv, dayDate) },
                            ])}>···</button>
                        {:else}
                          <div class="tp-chip-actions">
                            <button class="tp-chip-action-btn" title="Edit event"
                              on:click|stopPropagation={() => onEditDateEvent(devEv)}>📝</button>
                            <button class="tp-chip-action-btn" title="Lesson note"
                              on:click|stopPropagation={() => openOrCreateLessonNoteForEvent(devEv, dayDate)}>📓</button>
                          </div>
                        {/if}
                      </div>
                    {/each}
  ...
  <!-- ── Context menu ───────────────────────────────────────────────────── -->
  {#if contextMenu}
    <div class="tp-context-menu" style="left:{contextMenu.x}px; top:{contextMenu.y}px;" role="menu">
      <button class="tp-ctx-item" on:click={contextEditNotes}>📝 Edit notes</button>
      <button class="tp-ctx-item" on:click={contextChangeColour}>🎨 Change colour</button>
      <div class="tp-ctx-divider"></div>
      <button class="tp-ctx-item tp-ctx-danger" on:click={contextRemoveSlot}>✕ Remove from slot</button>
    </div>
  {/if}

  <!-- ── Resizer ─────────────────────────────────────────────────────────── -->
  <div class="tp-resizer" class:tp-resizer--active={resizerActive}
    role="separator" aria-label="Resize notes panel" on:mousedown={onResizerMouseDown}>
    <div class="tp-resizer-handle"></div>
  </div>

  <!-- ── Week notes ─────────────────────────────────────────────────────── -->
  <div class="tp-week-notes" style="height:{notesHeight}px;">
    <div class="tp-week-notes-header">
      <span class="tp-week-notes-title">Week Notes</span>
    </div>
    <textarea class="tp-week-notes-textarea" placeholder="Notes for this week…"
      value={getNoteValue()}
      on:input={(e) => setNoteValue(e.currentTarget.value)}
    ></textarea>
  </div>
...
<style>
  .tp-week-view { display:flex; flex-direction:column; height:100%; overflow:hidden; background:var(--background-primary); font-family:var(--font-interface); container-type:inline-size; }
  ...
  .tp-header-identity { display:flex; flex-direction:column; gap:1px; min-width:180px; }
  ...
  /* Table */
  .tp-grid { width:100%; border-collapse:collapse; table-layout:fixed; }
  ...
  /* Time cell */
  .tp-td-time { padding:4px 6px 0 8px; font-size:11px; color:var(--text-muted); vertical-align:top; white-space:nowrap; border:var(--grid-weight,1px) solid var(--grid-colour,var(--background-modifier-border)); background:var(--background-secondary); }

  /* Period label cell */
  .tp-td-period { padding:0; vertical-align:top; border:var(--grid-weight,1px) solid var(--grid-colour,var(--background-modifier-border)); }
  ...
  /* Day cells */
  .tp-td-cell { position:relative; padding:3px; border:var(--grid-weight,1px) solid var(--grid-colour,var(--background-modifier-border)); vertical-align:top; transition:background 0.1s; overflow:hidden; }
  .tp-td-cell--today    { background:color-mix(in srgb,var(--interactive-accent) 5%,transparent) !important; }
  ...
  /* Show chip action bar on individual chip hover (needed for column layout) */
  :global(.tp-chip:hover .tp-chip-actions) { opacity:1; pointer-events:auto; }

  /* Special event row */
  .tp-special-row { min-height:56px; }
  .tp-special-chip { cursor:pointer; }

  /* Action menu (short-row ••• button) */

  /* Short-row ··· dots button — tiny corner indicator, not a full button bar */
  .tp-chip-dots { position:absolute; bottom:2px; right:2px; padding:0 3px 1px; font-size:11px; line-height:1; background:rgba(0,0,0,0.18); color:var(--text-muted); border:none; border-radius:3px; cursor:pointer; opacity:0; transition:opacity 0.12s; z-index:4; letter-spacing:1px; pointer-events:none; }
  :global(.tp-chip:hover .tp-chip-dots) { opacity:1; pointer-events:auto; }

  /* Context menu */
  .tp-context-menu { position:fixed; z-index:500; background:var(--background-primary); border:1px solid var(--background-modifier-border); border-radius:6px; box-shadow:0 4px 16px rgba(0,0,0,0.28); padding:4px; min-width:170px; display:flex; flex-direction:column; gap:1px; }
  .tp-ctx-item { display:block; width:100%; padding:6px 10px; border:none; background:transparent; color:var(--text-normal); font-size:13px; font-family:var(--font-interface); text-align:left; cursor:pointer; border-radius:4px; transition:background 0.1s; }
  .tp-ctx-item:hover { background:var(--background-modifier-hover); }
  .tp-ctx-danger { color:var(--color-red,#f38ba8); }
  .tp-ctx-divider { height:1px; background:var(--background-modifier-border); margin:2px 0; }

  /* Resizer */
  .tp-resizer { flex-shrink:0; height:8px; cursor:ns-resize; display:flex; align-items:center; justify-content:center; background:var(--background-secondary); border-top:1px solid var(--background-modifier-border); border-bottom:1px solid var(--background-modifier-border); transition:background 0.1s; user-select:none; }
  .tp-resizer:hover, .tp-resizer--active { background:color-mix(in srgb,var(--interactive-accent) 15%,transparent); }
  .tp-resizer-handle { width:32px; height:3px; border-radius:2px; background:var(--background-modifier-border); }
  .tp-resizer:hover .tp-resizer-handle, .tp-resizer--active .tp-resizer-handle { background:var(--interactive-accent); }

  /* Week notes */
  .tp-week-notes { flex-shrink:0; display:flex; flex-direction:column; overflow:hidden; }
  .tp-week-notes-header { padding:5px 16px 3px; background:var(--background-secondary); border-bottom:1px solid var(--background-modifier-border); flex-shrink:0; }
  .tp-week-notes-title { font-size:12px; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.06em; }
  .tp-week-notes-textarea { flex:1; resize:none; width:100%; box-sizing:border-box; padding:8px 16px; background:var(--background-primary); color:var(--text-normal); border:none; outline:none; font-family:var(--font-text); font-size:14px; line-height:1.5; }

  /* Narrow-screen rotation: Time and Period columns only */
  @container (max-width: 500px) {
    .tp-col-time   { width:28px; }
    .tp-col-period { width:28px; }
